...
async function performSearch() {
  ...
  const filters = {
    minRating: ratingEl && Number(ratingEl.value) > 0 ? ratingEl.value : '',
  };
  ...
  try {
    ...
    updateQueryViewer(data.meta?.pipeline);
  } catch (err) {
...
// ─── Query Viewer ─────────────────────────────────────────────
function updateQueryViewer(pipeline) {
  if (!pipeline) return;
  const code = document.getElementById('queryCode');
  const str = JSON.stringify(pipeline, null, 2)
...
function copyQuery() {
  const text = document.getElementById('queryCode').textContent;
