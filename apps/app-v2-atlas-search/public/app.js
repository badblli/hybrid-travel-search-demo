...
const state = {
  ...
  queryViews: { results: null, facets: null, response: null },
  activeQueryTab: 'results',
};
...
async function performSearch() {
  ...
  try {
    ...
    updateQueryViewer({
      results: data.meta?.pipeline || null,
...
async function sendRagQuery() {
  ...
  // Also trigger search for the results grid
  performSearch();
  ...
  try {
    ...
    updateQueryViewer({
      results: data.meta?.pipeline || data.meta?.pipelines || null,
      facets: data.meta?.facetPipeline || null,
      response: data.meta?.facetResponse || null,
    });
...
function switchQueryTab(tab) {
  ...
  document.querySelectorAll('.query-tab').forEach((el) => {
    el.classList.toggle('active', el.id === `queryTab${tab.charAt(0).toUpperCase()}${tab.slice(1)}`);
  });
  renderActiveQueryTab();
}
